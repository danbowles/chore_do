defmodule ChoreDoWeb.HouseholdLive.FormComponent do
  ...
  def update(%{household: household} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Households.change_household(household))
     end)}
  end

  @impl true
  def handle_event("validate", %{"household" => household_params}, socket) do
    changeset = Households.change_household(socket.assigns.household, household_params)
  ...
  defp save_household(socket, :new, household_params) do
    case Households.create_household(household_params) do
      {:ok, household} ->
        notify_parent({:saved, household})
