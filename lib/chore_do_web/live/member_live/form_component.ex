defmodule ChoreDoWeb.MemberLive.FormComponent do
  use ChoreDoWeb, :live_component
    ...
    <div>
      ...
      >
        <.input
          ...
          options={Ecto.Enum.values(ChoreDo.Households.Member, :role)}
        />
        <:actions>
  ...
  def update(%{member: member} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Households.change_member(member))
     end)}
  end

  @impl true
  def handle_event("validate", %{"member" => member_params}, socket) do
    changeset = Households.change_member(socket.assigns.member, member_params)
  ...
  defp save_member(socket, :edit, member_params) do
    case Households.update_member(socket.assigns.member, member_params) do
      {:ok, member} ->
  ...
  defp save_member(socket, :new, member_params) do
    case Households.create_member(member_params) do
      {:ok, member} ->
        notify_parent({:saved, member})
